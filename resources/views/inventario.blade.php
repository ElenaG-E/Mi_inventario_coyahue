        ...
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    ...
                    <div id="qr-reader-results" class="mt-3 text-muted">Esperando código QR...</div>
                    <div id="qr-reader-error" class="text-danger mt-2"></div>
                </div>
    ...
    <div class="modal fade" id="modalReportes" tabindex="-1" aria-labelledby="modalReportesLabel" aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <form method="GET" action="{{ route('inventario.exportar') }}" target="_blank" id="formExportar">
                        <div class="mb-3">
                            <label class="form-label fw-bold">
                                <i class="fas fa-filter me-1"></i>Tipo de Contenido
                            </label>
                            <select name="tipo_reporte" class="form-select" required>
                                <option value="">Seleccionar...</option>
                                <option value="general">📊 Inventario Completo (Equipos + Insumos)</option>
                                <option value="equipos">💻 Solo Equipos</option>
                                <option value="insumos">📦 Solo Insumos</option>
                                <option value="asignaciones">👥 Asignaciones por Usuario</option>
                                <option value="sucursales">🏢 Inventario por Sucursal</option>
                                <option value="estadisticas">📈 Estadísticas Generales</option>
                            </select>
                            <small class="text-muted">
                                <i class="fas fa-info-circle me-1"></i>
                                Se exportarán los datos según los filtros actuales aplicados
                            </small>
                        </div>

                        <div class="mb-3">
                            <label class="form-label fw-bold">
                                <i class="fas fa-file-alt me-1"></i>Formato de Exportación
                            </label>
                            <div class="row g-2">
                                <div class="col-6">
                                    <div class="form-check form-check-inline w-100">
                                        <input class="form-check-input" type="radio" name="formato" id="formatoPdf" value="pdf" required>
                                        <label class="form-check-label w-100" for="formatoPdf">
                                ...
                                <div class="col-6">
                                    <div class="form-check form-check-inline w-100">
                                        <input class="form-check-input" type="radio" name="formato" id="formatoExcel" value="excel" required>
                                        <label class="form-check-label w-100" for="formatoExcel">
                                            <div class="card h-100">
                                                <div class="card-body text-center py-3">
                        ...
                        <div class="mb-3">
                            ...
                            <div class="form-check">
                                <input class="form-check-input" type="checkbox" name="incluir_precios" id="incluirPrecios" value="1" checked>
                                <label class="form-check-label" for="incluirPrecios">
                                    Incluir información de precios
                                </label>
                            </div>
                            <div class="form-check">
                                <input class="form-check-input" type="checkbox" name="incluir_fechas" id="incluirFechas" value="1" checked>
                                <label class="form-check-label" for="incluirFechas">
                                    Incluir fechas de registro/compra
                                </label>
                            </div>
                            <div class="form-check">
                                <input class="form-check-input" type="checkbox" name="incluir_proveedores" id="incluirProveedores" value="1">
                                <label class="form-check-label" for="incluirProveedores">
                        ...
                        <input type="hidden" name="filtro_usuario" value="{{ request('usuario') }}">
                        <input type="hidden" name="filtro_proveedor" value="{{ request('proveedor') }}">
                        <input type="hidden" name="filtro_sucursal" value="{{ request('sucursal') }}">
                        <input type="hidden" name="filtro_buscar" value="{{ request('buscar') }}">

                        <div class="alert alert-info mb-0">
                            <i class="fas fa-lightbulb me-2"></i>
                            <strong>Tip:</strong> Los filtros actualmente aplicados en la tabla se incluirán en la exportación.
                        </div>
                    </form>
                </div>
                <div class="modal-footer">
                    ...
                    <button type="submit" form="formExportar" class="btn btn-primary">
    ...
    @push('scripts')
        ...
        <script>
            document.addEventListener('DOMContentLoaded', function () {
                ...
                const usuarios = @json($usuarios->map(function($user) { 
                    return ['id' => $user->id, 'nombre' => $user->nombre]; 
                }));
                const sucursales = @json($sucursales->map(function($suc) { 
                    return ['id' => $suc->id, 'nombre' => $suc->nombre]; 
                }));
                            ...
                            (decodedText, decodedResult) => {
                                document.getElementById('qr-reader-results').innerHTML = 
                                    `<div class="alert alert-success">
                                        <i class="fas fa-check-circle me-2"></i>Código detectado: <strong>${decodedText}</strong>
                                    </div>`;
                                
                                setTimeout(() => {
                                    detenerScanner();
                                    buscarPorQR(decodedText);
                                }, 1000);
                            },
                            (errorMessage) => {
                                // Error normal durante escaneo
                            }
                        ).catch((err) => {
                            document.getElementById('qr-reader-error').innerHTML = 
                                `<i class="fas fa-exclamation-triangle me-2"></i>Error al iniciar cámara: ${err}`;
                        });
                    }
                }, { once: true });
            }
            ...
            function detenerScanner() {
                if (html5QrcodeScanner) {
                    html5QrcodeScanner.stop().then(() => {
                        ...
                        document.getElementById('qr-reader-results').innerHTML = 'Esperando código QR...';
